...
The `/api/v1/glossary/*` endpoints expose active-run pool metadata. They require a run in progress and are scoped to the current run/character context, not profile-wide discovered content. If no run is active, they return HTTP 409 with `error_code: "run_not_in_progress"`.
